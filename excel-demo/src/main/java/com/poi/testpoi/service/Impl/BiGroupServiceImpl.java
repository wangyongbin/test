...
import com.poi.testpoi.repository.BiGroupCompleteRepository;
import com.poi.testpoi.service.BiDeptService;
import com.poi.testpoi.service.BiGroupCompleteService;
...
@Service
public class BiGroupServiceImpl implements BiGroupCompleteService {

	@Autowired
	private BiGroupCompleteRepository biGroupCompleteRepository;
	...
	public boolean batchImport(String fileName, MultipartFile file) throws Exception {
		...
		fileName = "D:\\IdeaProjects\\my\\test\\excel-demo\\src\\main\\resources\\import_group.xls";
		// 加载excel文件
		...
		String jsonName = "D:\\IdeaProjects\\my\\test\\excel-demo\\src\\main\\resources\\import_group.json";
		// 加载json文件
		...
		for (SheetModel sheetModel : sheets){
			...
			for (DataAreaModel dataAreaModel : dataAreaModels){
				...
				if (tableName.equalsIgnoreCase("biGroupComplete")){
					List<BiGroupComplete> biGroupCompletes = getGroupCompleteData(sheet,data,errors,sheetIndex);
					...
					if (!errors.isEmpty()){
						...
						for (BiGroupComplete bdcd : biGroupCompletes){
	...
	private List<BiGroupComplete> getGroupCompleteData(Sheet sheet, DataModel dataModel, List<String> errors, int sheetIndex) {
		...
		BiGroupComplete biDeptCompleteDetail = null;
		List<BiGroupComplete> list = new ArrayList<>();
		...
		for (int i = rowStartIndex;i <= rowEndIndex;i++){
			...
			if (sheetIndex==4){
				...
				int yearTargetIndex = (int) columns.get("groupTarget");
				...
				biDeptCompleteDetail = new BiGroupComplete();
				...
				if (errors.isEmpty()){
					...
					biDeptCompleteDetail.setGroupTarget((Double) yearTargetCellVal);
