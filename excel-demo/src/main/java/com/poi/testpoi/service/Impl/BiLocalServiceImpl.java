...
@Service
public class BiLocalServiceImpl implements BiLocalCompleteService {

	@Autowired
	private BiLocalCompleteRepository biLocalCompleteRepository;
	...
	public boolean batchImport(String fileName, MultipartFile file) throws Exception {
		...
		fileName = "D:\\IdeaProjects\\my\\test\\excel-demo\\src\\main\\resources\\import_local.xls";
		// 加载excel文件
		...
		String jsonName = "D:\\IdeaProjects\\my\\test\\excel-demo\\src\\main\\resources\\import_local.json";
		// 加载json文件
		...
		for (SheetModel sheetModel : sheets){
			...
			for (DataAreaModel dataAreaModel : dataAreaModels){
				...
				if (tableName.equalsIgnoreCase("biLocalComplete")){
					List<BiLocalComplete> biLocalCompletes = getLocalCompleteData(sheet,data,errors,sheetIndex);
					...
					if (!errors.isEmpty()){
						...
						for (BiLocalComplete bdcd : biLocalCompletes){
	...
	private List<BiLocalComplete> getLocalCompleteData(Sheet sheet, DataModel dataModel, List<String> errors, int sheetIndex) {
		...
		BiLocalComplete biDeptCompleteDetail = null;
		List<BiLocalComplete> list = new ArrayList<>();
		...
		for (int i = rowStartIndex;i <= rowEndIndex;i++){
			...
			if (sheetIndex==4){
				...
				biDeptCompleteDetail = new BiLocalComplete();
